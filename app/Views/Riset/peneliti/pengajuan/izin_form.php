...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold text-dark mb-1">Form Izin Penelitian</h4>
        <p class="text-muted small mb-0">Lengkapi data pendaftaran izin penelitian Anda.</p>
    </div>
</div>
...
<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
    <div class="card-header bg-white py-3 px-4 border-bottom border-light">
        <div class="d-flex align-items-center text-danger">
            ...
            <h6 class="fw-bold mb-0" style="font-size: 14px;">Pengajuan Izin Penelitian</h6>
        </div>
    </div>
    <div class="card-body p-4">

        <form action="<?= base_url('riset/peneliti/pengajuan/izin/submit') ?>" method="post" enctype="multipart/form-data">
            ...
            <!-- Research Title -->
            <div class="mb-3">
                <label class="fw-bold text-dark mb-1" style="font-size: 13px;">Judul Izin Penelitian</label>
                <input type="text" name="judul" value="<?= $data['judul'] ?? '' ?>" class="form-control rounded-3 py-2" style="font-size: 13px; border-color: #eee;" placeholder="Masukkan judul izin penelitian anda" required oninvalid="this.setCustomValidity('Mohon isi judul izin penelitian Anda.')" oninput="this.setCustomValidity('')">
            </div>
            ...
            <div class="d-flex justify-content-end">
                <?php if (isset($data['id'])): ?>
                    ...
                    <button type="submit" class="btn btn-warning px-4 py-2 rounded-3 shadow fw-bold d-flex align-items-center text-dark" style="border: none; font-size: 14px; letter-spacing: 0.5px;">
                        Kirim Revisi <i class="fas fa-paper-plane ms-2"></i>
                    </button>
                <?php else: ?>
